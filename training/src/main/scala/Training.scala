...
object Training {
    ...
    def main() = {
      ...
      case class Features(maiScore: Double, deviceMatch: Double, factorCodes: Double, firstEncounter: Double)
      
      //Vector assembler tipado
      val assembler1 = TypedVectorAssembler[Features]

      case class SimpleClassWithFeatures(maiScore: Double, deviceMatch: Double, factorCodes: Double, firstEncounter: Double, apocrypha: Double, features: Vector)
      //Cómo seteo esto en un pipeline????? a menos que sea un preproceso
      val trainingDataWithFeatures = assembler1.transform(trainingData).as[SimpleClassWithFeatures]
 
      def assembler() : PipelineStage = {
        val a = TypedVectorAssembler[Features]
        a.transformer
        .setInputCols(Array("maiScore","deviceMatch", "factorCodes","firstEncounter"))
      ...
      val modelAssembled2 = new Pipeline().setStages(List(assembler(), rFclassifier("apocrypha")).toArray)

      val pmml2z = new PipelineSaver().toPmml(modelAssembled2.fit(trainingData.toDF()), trainingData.toDF)
